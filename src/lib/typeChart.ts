// Gen 8 type chart — no Steel resistance to Ghost/Dark (that ended after Gen 5).
...
/**
 * For a defender with the given type(s), returns the damage multiplier each
 * of the 18 attacking types would deal (product across all defender types).
 */
export function getAttackingEffectiveness(
  ...
  for (const attackType of TYPES) {
    ...
    for (const defenderType of defenderTypes) {
      ...
      multiplier *= chart[defenderType as PokemonType] ?? 1;
    }
