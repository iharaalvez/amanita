...
export const DONUT_FLAVORS: {
  ...
  },
  {
    ...
    powers: ["Item", "Mega Charging", "Mega Conservation", "Big Haul"],
  },
  {
    id: "bitter",
    label: "Bitter",
    className: "bg-violet-500",
    powers: ["Defense", "Sp. Defense", "Resistance"],
  },
  {
    id: "fresh",
    label: "Fresh",
    className: "bg-sky-500",
    powers: ["Encounter", "Capture"],
  },
];
...
// Totals are calculated from the berry data above in the UI.
export const RECOMMENDED_DONUT_RECIPES: RecommendedDonutRecipe[] = [
  {
    id: "shiny-haban-8",
    title: "Sweet Shiny Base",
    category: "Shiny Hunting",
    description:
      "High Sweet with Fresh for Sparkling, Alpha, and possible Encounter rolls.",
    ingredients: [{ berryId: "hyper-haban-berry", count: 8 }],
  },
  {
    id: "shiny-haban-tanga",
    title: "Max Sweet Hunt",
    category: "Shiny Hunting",
    description: "Very high Sweet for chasing Sparkling and Alpha Power rolls.",
    ingredients: [
  ...
  },
  {
    ...
    title: "Balanced Sweet Hunt",
    category: "Shiny Hunting",
    description: "Sweet-forward recipe with some Fresh coverage.",
    ingredients: [
  ...
  {
    id: "item-kasib-8",
    title: "Pure Sour Haul",
    category: "Item Farming",
    description: "Sour-focused recipe for Big Haul and Item Power attempts.",
    ingredients: [{ berryId: "hyper-kasib-berry", count: 8 }],
  },
  {
    id: "item-kasib-roseli",
    title: "Sour Item Farm",
    category: "Item Farming",
    description: "Strong Sour recipe using Kasib and Roseli.",
    ingredients: [
  ...
  },
  {
    ...
    title: "Mixed Sour Farm",
    category: "Item Farming",
    description: "Sour-heavy recipe with a broader berry mix.",
    ingredients: [
      { berryId: "hyper-kasib-berry", count: 4 },
  ...
  },
  {
    ...
    title: "Hyper Berry Farm Starter",
    category: "Berry Farming",
    description:
      "Cost-efficient Sour setup; add Babiri or Roseli as an optional extra berry.",
    ingredients: [
  ...
  },
  {
    ...
    category: "Story",
    description: "Simple early recipe from regular berries.",
    ingredients: [{ berryId: "pecha-berry", count: 3 }],
  },
  {
    id: "story-early-mixed",
    title: "Early Mixed Donut",
    category: "Story",
    description: "Regular-berry option with more flavor variety.",
    ingredients: [
  ...
  },
  {
    ...
    title: "Late Portal Push",
    category: "Story",
    description:
      "High-level late-game recipe with strong Spicy and Bitter totals.",
    ingredients: [
...
export function getBudgetForPoints(
  ...
  for (const threshold of thresholds) {
    if (points >= threshold.points) budget = threshold.budget;
  }
  return budget;
}

export function getDominantFlavors(totals: DonutTotals): DonutFlavor[] {
  const max = Math.max(...DONUT_FLAVORS.map((flavor) => totals[flavor.id]));
  if (max <= 0) return [];
  return DONUT_FLAVORS.filter((flavor) => totals[flavor.id] === max).map(
    (flavor) => flavor.id,
  );
}
