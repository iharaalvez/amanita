...
import type { LivingDexEntry, OwnedRecord, OwnershipMethod, ProgressSnapshot, ShinyHuntMethod } from '@/types/pokemon';
...
function downloadCSV(owned: Record<string, OwnedRecord>, entries: LivingDexEntry[]) {
  const nameMap = new Map(entries.map((e) => [ownedKey(e.speciesId, e.formName), e.displayName]));
  const header = 'speciesId,formName,name,owned,shiny_owned,method,game_caught,shiny_method,shiny_game,planned';
...
function SectionHeader({ icon, title, accent }: { icon?: string; title: string; accent: string }) {
  return (
    <div className={`flex items-center gap-2 mb-5 pb-3 border-b-2 ${accent}`}>
      ...
      <h2 className="text-base font-bold dark:text-white tracking-tight">{title}</h2>
    </div>
  );
}
...
export function StatsView() {
  ...
  const stats = useMemo(() => {
    ...
    const genData = GENERATIONS.map((gen) => {
      ...
      return { gen, label: GEN_LABELS[gen], total: genTotal, owned: genOwned, shiny: genShiny };
    });

    const methodCounts: Record<OwnershipMethod, number> = {
      caught: 0, bred: 0, hatched: 0, transferred: 0, event: 0,
    };
    ...
    for (const r of records) {
      if (r.shiny_owned && r.shiny_method) {
        shinyMethodCounts[r.shiny_method] = (shinyMethodCounts[r.shiny_method] ?? 0) + 1;
      }
    }
    const taggedShiny = Object.values(shinyMethodCounts).reduce((a, b) => a + b, 0);
    const untaggedShiny = shinyCount - taggedShiny;

    return {
      total, ownedCount, shinyCount,
      genData, methodCounts, untaggedOwned,
      shinyMethodCounts, untaggedShiny,
      maxMethodCount,
    };
  ...
  async function handleImport(file: File | undefined) {
    ...
    try {
      ...
      if (!isProgressSnapshot(parsed)) {
        setImportMessage('That file does not look like a Living Pokedex backup.');
        return;
      }
      const importedOwnedCount = Object.values(parsed.owned).filter((record) => record.owned).length;
      const confirmed = window.confirm(
        `Restore this backup and replace current local data?\n\nOwned records: ${importedOwnedCount}`
      );
      if (!confirmed) return;
      setProgressSnapshot(parsed);
      setImportMessage(`Backup restored. ${importedOwnedCount} owned records loaded.`);
    } catch {
    ...
    <div className="max-w-3xl mx-auto px-4 pt-6 pb-16 space-y-12">
      ...
      <section>
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-5">
          <div className="rounded-xl bg-green-50 p-4 text-center dark:bg-green-950/50 sm:col-span-2 border border-green-100 dark:border-green-900">
            <p className="text-3xl font-bold tabular-nums text-green-600 dark:text-green-400">{ownedPct.toFixed(1)}%</p>
          ...
          <div className="rounded-xl bg-green-50 dark:bg-green-950/50 border border-green-100 dark:border-green-900 p-4 text-center">
            <p className="text-2xl font-bold tabular-nums text-green-600 dark:text-green-400">{stats.ownedCount}</p>
          ...
          <div className="rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-4 text-center">
            <p className="text-2xl font-bold tabular-nums dark:text-white">{stats.total}</p>
          ...
          <div className="rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/50 p-4 text-center">
            <p className="text-2xl font-bold tabular-nums text-red-400">{stats.total - stats.ownedCount}</p>
        ...
        <div className="mb-1 flex justify-between text-xs text-gray-400">
          ...
          <span className="tabular-nums font-medium">{ownedPct.toFixed(1)}%</span>
        </div>
        ...
        <div className="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 divide-y divide-gray-50 dark:divide-gray-700 mb-6">
          {stats.genData.map(({ gen, label, total, owned }) => {
            ...
            return (
              <div key={gen} className="px-4 py-3 grid grid-cols-[52px_1fr_64px] items-center gap-4">
                <span className="text-xs font-semibold text-gray-400">Gen {label}</span>
                <HBar pct={pct} colorClass="bg-green-400" />
                <span className="text-xs tabular-nums text-right text-gray-500 dark:text-gray-400">
                  {owned}<span className="text-gray-300 dark:text-gray-600">/</span>{total}
                </span>
              </div>
            );
          })}
        ...
        {/* Acquisition methods */}
        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">How you caught them</h3>
        {stats.ownedCount === 0 ? (
          ...
          <div className="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 divide-y divide-gray-50 dark:divide-gray-700">
            {OWNERSHIP_METHODS.filter(({ value }) => stats.methodCounts[value] > 0).map(({ value, label }) => {
              const count = stats.methodCounts[value];
              const pct = (count / stats.maxMethodCount) * 100;
              return (
                <div key={value} className="px-4 py-3 grid grid-cols-[88px_1fr_32px] items-center gap-3">
                  <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{label}</span>
                  <HBar pct={pct} colorClass="bg-green-400" />
                  <span className="text-xs tabular-nums text-right text-gray-400">{count}</span>
                </div>
              );
            })}
            {stats.untaggedOwned > 0 && (
              <div className="px-4 py-3 grid grid-cols-[88px_1fr_32px] items-center gap-3">
                <span className="text-xs font-medium text-gray-400 italic">Untagged</span>
                <HBar pct={(stats.untaggedOwned / stats.maxMethodCount) * 100} colorClass="bg-gray-200 dark:bg-gray-600" />
                <span className="text-xs tabular-nums text-right text-gray-400">{stats.untaggedOwned}</span>
              </div>
            )}
          </div>
        )}
      ...
      <section>
        <div className="mb-5 flex items-center gap-2 border-b-2 border-yellow-400 pb-3">
          ...
          <h2 className="text-base font-bold tracking-tight dark:text-white">Shiny Living Dex</h2>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="rounded-xl bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-100 dark:border-yellow-900/50 p-4 text-center">
            <p className="text-2xl font-bold tabular-nums text-yellow-500">{stats.shinyCount}</p>
        ...
        <div className="mb-1 flex justify-between text-xs text-gray-400">
          <span>Overall progress</span>
          <span className="tabular-nums font-medium">{shinyPct.toFixed(1)}%</span>
        </div>
        <div className="h-3 rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden mb-6">
          <div
        ...
        {stats.shinyCount > 0 && (
          <div className="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 divide-y divide-gray-50 dark:divide-gray-700 mb-6">
            {stats.genData.filter(({ shiny }) => shiny > 0).map(({ gen, label, total, shiny }) => {
              ...
              return (
                <div key={gen} className="px-4 py-3 grid grid-cols-[52px_1fr_64px] items-center gap-4">
                  ...
                  <span className="text-xs tabular-nums text-right text-yellow-500">
                    {shiny}<span className="text-gray-300 dark:text-gray-600">/</span><span className="text-gray-400">{total}</span>
                  </span>
                </div>
              );
            })}
        ...
        {/* Hunt methods */}
        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Hunt methods</h3>
        {stats.shinyCount === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-200 dark:border-gray-700 p-8 text-center text-sm text-gray-400 italic">
            No shinies logged yet - mark a Pokemon as shiny owned in the detail panel
          </div>
        ) : (
          <div className="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 divide-y divide-gray-50 dark:divide-gray-700">
            {SHINY_METHODS.map(({ value, label, odds }) => {
              ...
              return (
                <div key={value} className="px-4 py-3 grid grid-cols-[96px_1fr_48px] items-center gap-3">
                  <div>
                    <p className="text-xs font-medium text-gray-500 dark:text-gray-400">{label}</p>
                    <p className="text-[10px] text-gray-300 dark:text-gray-600">{odds}</p>
                  </div>
                  <HBar pct={pct} colorClass="bg-yellow-400" />
                  <span className="text-xs tabular-nums text-right text-yellow-500 font-medium">* {count}</span>
                </div>
              );
            })}
            {stats.untaggedShiny > 0 && (
              <div className="px-4 py-3 grid grid-cols-[96px_1fr_48px] items-center gap-3">
                <span className="text-xs font-medium text-gray-400 italic">Untagged</span>
                <HBar pct={(stats.untaggedShiny / stats.shinyCount) * 100} colorClass="bg-gray-200 dark:bg-gray-600" />
                <span className="text-xs tabular-nums text-right text-yellow-500">* {stats.untaggedShiny}</span>
              </div>
            )}
          </div>
        )}
      ...
      {/* Export / Backup */}
      <section>
        <SectionHeader title="Export / Backup" accent="border-gray-300 dark:border-gray-600" />
        <p className="text-xs text-gray-400 mb-4">
          Download your Pokedex data to back it up, or restore a previous JSON backup.
        </p>
        <input
          ref={fileInputRef}
          type="file"
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          >
            ...
            <span className="text-xs font-normal text-gray-400">Full backup - reimportable</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-xs font-normal text-gray-400">Spreadsheet-friendly</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-xs font-normal text-gray-400">Restore backup</span>
          </button>
        </div>
        {importMessage && <p className="mt-3 text-xs text-gray-400">{importMessage}</p>}
      </section>
