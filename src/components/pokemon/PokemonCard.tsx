...
import { ArrowUpRightIcon, BookmarkIcon, HomeIcon, SparkleIcon } from '@/components/ui';
  ...
  return (
    <div className="relative flex flex-col">
      ...
      >
        ...
        <span className="text-[10px] tabular-nums text-gray-400">{paddedNumber}</span>

        <span className="w-full truncate px-1 text-center text-[11px] font-medium leading-tight">
          {formName ? displayName.split(' ').slice(1).join(' ') || displayName : displayName}
        </span>

        {formLabel && (
          <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-semibold leading-none ${REGION_TAG_CLASSES[formLabel] ?? 'bg-teal-100 text-teal-700 dark:bg-teal-900 dark:text-teal-300'}`}>
            {formLabel}
          </span>
        )}
      </button>
