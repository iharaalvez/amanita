    ...
    <Tooltip content={label}>
      ...
      >
        <span className="absolute right-1.5 top-1.5 flex gap-1" aria-hidden>
          ...
          {owned && !inHome && <ArrowUpRightIcon className="h-3.5 w-3.5 text-blue-400" />}
          {!owned && planned && <BookmarkIcon className="h-3.5 w-3.5 text-violet-400" />}
        </span>
        <Image
          src={entry.spriteUrl}
        ...
        />
        <span className="w-full truncate px-1 text-center text-[9px] font-medium leading-tight text-gray-500 dark:text-gray-300 sm:text-[10px]">
          {entry.displayName}
        </span>
      </button>
    </Tooltip>
