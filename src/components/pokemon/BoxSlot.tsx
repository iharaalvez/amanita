...
import { ArrowUpRightIcon, BookmarkIcon, HomeIcon, SparkleIcon, Tooltip } from '@/components/ui';
...
export function BoxSlot({ entry, onSelect }: Props) {
  ...
  const shinyOwned = usePokedexStore((s) => (entry ? !!s.owned[key]?.shiny_owned : false));
  const inHome = usePokedexStore((s) => (entry ? !!s.owned[key]?.in_home : false));
  const planned = usePokedexStore((s) => (entry ? !!s.owned[key]?.planned : false));
    ...
    <Tooltip content={label}>
      <div className="relative aspect-square w-full">
        <button
          ...
          className={`group absolute inset-0 flex cursor-pointer flex-col items-center justify-center gap-0.5 overflow-hidden rounded-lg p-1 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 ${ringClass}`}
        >
          <span className="absolute right-1 top-1 flex gap-0.5" aria-hidden>
            {shinyOwned && <SparkleIcon className="h-3 w-3 text-yellow-400" />}
            {inHome && <HomeIcon className="h-3 w-3 text-green-400" />}
            {owned && !inHome && <ArrowUpRightIcon className="h-3 w-3 text-blue-400" />}
            {!owned && planned && <BookmarkIcon className="h-3 w-3 text-violet-400" />}
          </span>
          <Image
            src={entry.spriteUrl}
            ...
            className={`h-8 w-8 flex-shrink-0 object-contain transition-all duration-200 group-hover:scale-110 sm:h-10 sm:w-10 ${
              owned || inHome || shinyOwned ? '' : 'grayscale opacity-50'
            }`}
          />
          <span className="w-full truncate px-0.5 text-center text-[9px] font-medium leading-none text-gray-500 dark:text-gray-300">
            {entry.displayName}
          </span>
        </button>
      </div>
